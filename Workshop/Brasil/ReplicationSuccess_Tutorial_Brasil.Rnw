...
For all studies from the replication projects investigate
  \begin{block}{Exercise 1.3}
    Compute the one-sided \textbf{sceptical $p$-value} (with golden 
    and controlled recalibration). How many replication 
  ...
  \begin{block}{Exercise 1.4}
    ...
    success based on the two-trials rule and the sceptical $p$-value (golden). How do their 
