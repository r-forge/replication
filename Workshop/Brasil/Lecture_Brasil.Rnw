...
\def\uzhunit{}
\def\uzhunitext{}
\title{\centering The Statistical Assessment of 
  Replication Success}
\author{\centering \bf  Leonhard Held}
\institute{University of Zurich}

  
\date{July 31 - August 5, 2022}
...
\includegraphics[width=0.6\textwidth]{images_presentation/CRS.png}
  ...
  \end{columns}
    \begin{center}
    {\color{darkred} 24th SINAPE, Gramado}
    \end{center}
\end{frame}
...
\begin{block}{}
  \begin{itemize}
    \item<1->                {2015: Psychology reproducibility project}
...
\begin{block}{}
  \begin{itemize}
    \item<1-> \color<1>{gray}{2015: Psychology reproducibility project}
...
\begin{frame}
\section{Replication success}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Replication success}} 
   \end{center}
  \end{block}
\end{frame}
...
\begin{minipage}[t][3cm][t]{\textwidth}
\begin{block}{}
{\large \textit{Is the combined effect estimate statistically significant?}} 

$\rightarrow$ Which threshold? \\
$\rightarrow$ {\color{tan3} one-sided $\alpha = 0.025$ or $0.0025$} is often used\\
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
<< fig.height = 4.5, echo = FALSE >>=
...
\framesubtitle{Two-trials rule vs.~sceptical $p$-value}

\vspace{0cm}
%%\hspace{-.5cm}

<<typeIE-projPower-fixedc2b, fig.height = 5.5, cache = TRUE, echo=FALSE>>=
...
\begin{frame}[fragile]{The relative effect size}
  \framesubtitle{Pyc and Rawson (2010) example}%% For the assessment of replication success}
...
@
    ...
    \begin{itemize}
      ...
      \Sexpr{round(study$fisr/study$fiso, 2)}$ }
\visible<2->{\item Minimum relative effect size}
        \begin{itemize}
\visible<2->{  
...
\visible<4->{  
      \begin{center}
    {\color{darkred} When do the two methods disagree?}
    \end{center}
    }
...
%%      \begin{center}
%%      \fbox{\color{foo}Shrinkage $s = 1 - d$}
%%    \end{center}
...
\framesubtitle{Relative sample size $c \rightarrow \infty$}
  \vspace{1cm}
  \begin{center}
...
\section{Outlook}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Outlook}} 
   \end{center}
  \end{block}
\end{frame}

\begin{frame}{Real-world evidence studies}
\framesubtitle{\citet{Franklin_etal2021}}
  \vspace{-2cm}
  \begin{minipage}[t][5cm][t]{\textwidth}
  \centering
  \includegraphics[width=0.8\textwidth]{images_presentation/Circ.RCT-D.2021Top.png}\\[.5cm]
  \includegraphics[width=0.8\textwidth]{images_presentation/circulationaha.120.051718.fig02.png}
\end{minipage}
\end{frame}
...
\begin{frame}{Automated reproducibility testing}
\framesubtitle{\citet{Roper_etal2022}}
  \vspace{-2cm}
  \begin{minipage}[t][5cm][t]{\textwidth}
  \centering
  \includegraphics[width=0.8\textwidth]{images_presentation/Roper_etal2022Top.png}\\[.5cm]
  \includegraphics[width=0.6\textwidth]{images_presentation/rsif20210821f01.jpg}
\end{minipage}
\end{frame}
...
\begin{itemize}
  ...
  \item \alert{Multisite} replication studies
