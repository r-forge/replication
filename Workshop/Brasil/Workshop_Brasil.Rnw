...
\begin{block}{Some proposed criteria}
  ...
  \pause
  \begin{enumerate}
  {\color{black!25} \item[4.] Sceptical $p$-value (second part)}
  \end{enumerate}
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
...
\item [$\rightarrow$] {\color{red} Fisher's $z$-transformation} for correlation 
coefficients $r$ with (effective) sample size $n - 3$
\end{itemize}
\end{block}
...
\begin{tabular}{l l}
  ...
  \texttt{se\_fisr} & Standard error of \texttt{fisr} \\
   \texttt{po1} & One-sided original $p$-value \\
    \texttt{pr1} & One-sided replication $p$-value
\end{tabular}
\end{table}
\end{block}
\end{frame}

\begin{frame}[fragile]{Key quantities}
<< echo = FALSE >>=
po <- study$po
pr <- study$pr
...
\begin{block}{}
\begin{itemize}
\item $z$-value $z_o$ of original study
<< size = "small" >>=
RProjects$zo <- RProjects$fiso/RProjects$se_fiso
@
\pause
\item $z$-value $z_r$ of replication study
<< size = "small" >>=
RProjects$zr <- RProjects$fisr/RProjects$se_fisr
@
\pause
<<size = "small", eval = F>>=
# z-values can be transformed to p-values (and vice-versa)
p <- z2p(z, alternative = "one.sided")
z <- p2z(p, alternative = "one.sided")
@
...
\begin{block}{}
\begin{enumerate}
\item Visit {\small \url{https://gitlab.uzh.ch/charlotte.micheloud/replicationstudies}}
...
\begin{block}{}
\begin{enumerate}
\item[4.] Compute the key quantities $z_o$, $z_r$ and $c$ with
<<>>=
RProjects$zo <- RProjects$fiso/RProjects$se_fiso
RProjects$zr <- RProjects$fisr/RProjects$se_fisr
RProjects$c <- RProjects$se_fiso^2/RProjects$se_fisr^2
...
\item[5.] Explore the \texttt{ReplicationSuccess} package
<<eval = FALSE>>=
# for example
?RProjects
@
...
\begin{frame}{Important function}
  
  \begin{block}{Incompatibility of effect estimates}
  
  \begin{itemize}
    \item[] Function: \texttt{Qtest}
    \item[] Arguments
      \begin{itemize}
        \item \texttt{thetao}
        \item \texttt{thetar}
    ...
    \item[] Returns \texttt{p\_Q}, the $p$-value from the Q-test
  \end{itemize}
...
\begin{frame}[fragile]{Example from \citet{Pyc2010}}
  \begin{block}{}
    \begin{itemize}
      \item Effect estimates: $\hat\theta_o = \Sexpr{round(study$fiso, 2)}$, 
      $\hat\theta_r = \Sexpr{round(study$fisr, 2)}$
      \item Standard errors: $\sigma_o = \Sexpr{round(study$se_fiso, 2)}$, 
      $\sigma_r = \Sexpr{round(study$se_fisr, 2)}$
    \end{itemize}
    
<<>>=
Qtest(thetao = 0.4, thetar = 0.15, 
      seo = 0.17, ser = 0.06)
@
...
{\color{darkred} No evidence for incompatibility at level $\alpha = 5$\%}
...
$\alpha = 0.05$ for the {\bf two-sided $p_M$}.
  ...
  \end{block}
    ...
    \begin{block}{}
{\color{darkred} Useful function: \texttt{meta::metagen(...)\$pval.fixed}}
    \end{block}
\end{frame}
...
@
   \begin{block}{}
    \begin{itemize}
      \item $Q$-test $p$-value: $p_Q = \Sexpr{biostatUZH::formatPval(ex1.4$pQ)}$;
...
\begin{frame}{}
\begin{center}
  \color{uzh@blue}{\Huge \textbf{When is a replication successful?}} \\[1cm]
  \color{uzh@blue}{\Large Newly developed method}
   \end{center}
\end{frame}
...
\begin{column}{0.3\textwidth}
  % \centering
\includegraphics[width=1.5\textwidth]{images_presentation/Held2020.png}
...
\begin{column}{0.3\textwidth}
\includegraphics[width=1.3\textwidth]{images_presentation/Heldetal2021.png}
...
@
      ...
      \begin{block}{Golden $p_S$}
        ...
        \pause
        \item {\color{red} Necessary condition for replication success:}  \\
        $p_o$ and $p_r$
...
@
      ...
      \begin{block}{Controlled $p_S$}
        \begin{itemize}
          \item {\color{red} exact} overall Type-I error control {\color{red} at level 
        ...
        \pause
        \item Allows calculation of {\bf $p$-value function}
        and  {\bf sceptical confidence interval}.
        \end{itemize}
      \end{block}
...
% \end{block}
% \vspace{-1cm}
<<echo = F, fig.height = 3.3, fig.width = 4.5, eval = F>>=
par(las = 1, 
...
\begin{frame}{Important function}
  ...
  \begin{itemize}
    \item[] Function: \texttt{pSceptical}
    \item[] Arguments (\texttt{default})
      \begin{itemize}
        \item \texttt{zo}
        \item \texttt{zr}
        \item \texttt{c}
        \item \texttt{alternative ("one.sided")}
        \item \texttt{type ("golden")}
      \end{itemize}
  \end{itemize}
...
\begin{frame}[fragile]{Example from \citet{Pyc2010}}
  \begin{block}{}
    \begin{itemize}
  \item $z_o = \Sexpr{round(study$fiso/study$se_fiso, 2)} \longleftrightarrow p_o = \Sexpr{formatPval(study$po)}$
    \item $z_r = \Sexpr{round(study$fisr/study$se_fisr, 2)} \longleftrightarrow p_r = \Sexpr{formatPval(study$pr)}$
    \item $c = \Sexpr{round(study$c, 2)}$
    \end{itemize}
    
<<>>=
pSceptical(zo = 2.27, zr = 2.63, c = 9.2, alternative = "one.sided", 
...
{\color{darkred} Replication success with the controlled $p_S$ 
but not with the golden $p_S$ at level $\alpha = 0.025$}
...
\begin{frame}{Important function}
  ...
  \begin{block}{Sceptical confidence interval}
  
  \begin{itemize}
    \item[] Function: \texttt{scepticalCI}
    \item[] Arguments (\texttt{default})
      \begin{itemize}
        \item \texttt{thetao}
        \item \texttt{thetar}
        \item \texttt{seo}
        \item \texttt{ser}
        \item \texttt{levelCI (0.95)}
      \end{itemize}
      
    \item[] Returns a confidence \emph{interval} or a confidence \emph{set}
  \end{itemize}
...
<<>>=
scepticalCI(thetao = 0.4, thetar = 0.15, 
            seo = 0.17, ser = 0.06, 
            levelCI = 0.95)$CI
@
...
{\color{darkred} Sceptical confidence \emph{interval}}
...
\begin{frame}[fragile]{Example from \citet{Storm2008}}
...
@
  ...
  \begin{block}{}
    \begin{itemize}
      \item Effect estimates: $\hat\theta_o = \Sexpr{round(study2$fiso, 2)}$, 
      $\hat\theta_r = \Sexpr{round(study2$fisr, 2)}$
      \item Standard errors: $\sigma_o = \Sexpr{round(study2$se_fiso, 2)}$, 
      $\sigma_r = \Sexpr{round(study2$se_fisr, 2)}$
    \end{itemize}
    
<<>>=
scepticalCI(thetao = 0.23, thetar = -0.01, 
            seo = 0.07, ser = 0.06, 
            levelCI = 0.95)$CI
@
...
{\color{darkred} Sceptical confidence \emph{set}}
...
\begin{frame}{Important function}
  ...
  \begin{block}{p-value function}
  
  \begin{itemize}
    \item[] Function: \texttt{pValFunPlot}
    \item[] Arguments (\texttt{default})
      \begin{itemize}
        \item \texttt{thetao}
        \item \texttt{thetar}
    ...
    \item[] Plots the $p$-value function
  \end{itemize}
...
<<fig.height = 4>>=
pValFunPlot(thetao = 0.4, thetar = 0.15, 
            seo = 0.17, ser = 0.06, 
            levelCI = 0.95)
@
...
\end{block}
  \begin{block}{Exercise 2.3}
    ...
    Calculate the 95\% sceptical confidence interval. 
    \end{itemize}
  \end{block}
...
\begin{center}
{\color{gray} ``Compute the one-sided golden and controlled 
sceptical $p$-values. How many studies are successful at level $0.025$?''}
\end{center}
...
colnames(Table3) <- c("Project", "$p_S$ (golden) $\\leq 0.025$", 
                      "$p_S$ (controlled) $\\leq 0.025$")
...
\framesubtitle{Part c)}
  \begin{center}
  {\color{gray} ``Have a closer look at the studies for which the golden 
  and the controlled sceptical $p$-value do not lead to the same conclusion.''}
  \end{center}
...
colnames(discTab2) <- c("Study", "$c$", "$r_o$", "$r_r$", "$p_o$", 
...
\begin{block}{}
{\color{red} \small red: $p$-value larger than 0.025}
\end{block}
\end{frame}
...
\framesubtitle{Part a)}
  ...
  \begin{center}
  {\color{gray} ``Have a closer look at the studies for which the
  controlled sceptical $p$-value and the two-trials rule
...
colnames(discTab) <- c("Study", "$c$", "$r_o$", "$r_r$", "$p_o$", 
                       "$p_r$", "$p_s$ (controlled)")
...
\framesubtitle{Part b)}
  \begin{center}
  {\color{gray} ``Have a closer look at the studies for which the
  golden sceptical $p$-value and the two-trials rule
  do not lead to the same conclusion.''}
  \end{center}
\makebox[1 \textwidth][c]{  
  \resizebox{1.2\textwidth}{!} {
...
@
  \begin{center}
  {\color{red} \small min p is the minimum of the $p$-value function}
  \end{center}
\end{frame}
...
\framesubtitle{(Solutions: {\scriptsize \url{https://gitlab.uzh.ch/charlotte.micheloud/replicationstudies}})}
\begin{block}{}
For all the original studies from the replication projects
\end{block}
  \begin{block}{Exercise 3.3}
  Compute the required relative sample size to reach a power of 90\% 
  (both conditional and predictive). How do they compare to the 
  relative sample sizes which were used?
\end{block}  
...
\begin{frame}<handout:0>{Solution: Exercise 3.3}
...
@
  ...
  \begin{center}
  {\color{red} \small for a power of 90\%}
  \end{center}
...
\begin{frame}{Sample size calculation based on $p_S$}
  ...
  {\color{red} replication success with the sceptical $p$-value} (controlled or golden)
  ...
  \pause 
  \item Only depends on the {\color{red} original $z$-value $z_o$} (equivalently $p_o$),
  the level $\alpha$ and the desired 
  power.
  \end{itemize}
\end{frame}
...
\framesubtitle{(Solutions: {\scriptsize \url{https://gitlab.uzh.ch/charlotte.micheloud/replicationstudies}})}
  \begin{block}{Exercise 4.1}
    ...
    \item What happens if you use a relative sample size of $c = 2$?
    \end{itemize}
  \end{block}
...
Held2020, HeldMichPaw2021, MicheloudHeld2022, Errington2021}
