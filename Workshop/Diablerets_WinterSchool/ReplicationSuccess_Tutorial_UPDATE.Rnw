...
study_plot <- ggplot(data = plot_df, aes(x = Study, y = z)) +
  ...
  annotate("text", x = 1.5, y = study$ro, 
           ...
           color = "red") + 
  ylim(-0.05, 1) +
   labs(x = " ", y = expression(italic(hat(theta)))) + 
  theme_bw() + 
  theme(axis.text = element_text(size = 25), 
        axis.title = element_text(size = 30)) 
study_plot
...
\begin{frame}{Comparison of Success Regions}
  \framesubtitle{Relative sample size c = 0.5}
  \begin{centering}
\includegraphics[width = 0.9\textwidth]{images_presentation/comparison.png}
  \end{centering}
  
  CM: Here I would suggest to put the same plots as in the METRICS talk to show the 
  influence of the sample size on the success region. Maybe only for the golden level 
  instead of nominal level. (I unfortunately can't do it as I don't have plotLevel.R)
\end{frame}
