...
%%\includegraphics[width=2.5cm]{figures/Bishop.jpg} \\[.5cm]
    \includegraphics[width=7cm]{/home/leo/lehre/medpres/jahr1neu/slides/pictures/horsemen.jpg}\\
%\hspace{-2cm}
...
\end{center}
  ...
  \begin{minipage}{3cm}
\vspace{.5cm}
\includegraphics[width=3cm]{/home/leo/talks/EBPILunch2017/figures/Bishop.jpg}
\end{minipage}
\end{multicols}
...
\framesubtitle{in the assessment of replication success}
  ...
  \begin{block}{Pyc and Rawson example}
<<echo = FALSE>>=
...
\begin{frame}{Shrinkage of replication effect estimate}

Shrinkage {\color{foo} $s = 1 - d$} \\
    of replication effect estimate $\hat\theta_r$ 
    as compared to original effect estimate $\hat\theta_o$ is 
    {\color{red} penalized} in the sceptical $p$-value approach
...
<<echo = F, eval = F>>=
par(mfrow = c(1,1))
dmin_sign <- effectSizeSignificance(zo = RProjects$zo, c = RProjects$c, level = 0.025)
dmin_rs <- effectSizeReplicationSuccess(zo = RProjects$zo, c = RProjects$c, level = 0.025, 
                             type = "golden", alternative = "one.sided")

plot(dmin_sign, dmin_rs, pch = 20, xlim = c(0, 2), ylim = c(0, 2))
abline(coef = c(0,1), col = "red")
mean(dmin_sign < dmin_rs, na.rm = T)
mean(dmin_sign > dmin_rs, na.rm = T)
@
...
\begin{frame}{Power of the two-trials rule} 
  \framesubtitle{Alternative formulation}
  \begin{block}{Conditional design prior}
    \begin{eqnarray*}
...
\begin{frame}[fragile]{TODO}
  \begin{itemize}
    \item Introducing meta-analysis 
    \item Introducing multiple replication studies per original study
    \item Preparing an exercise with the dataset from Protzo
    \end{itemize}
<<>>=
data("protzko2020")
head(protzko2020, 10)
...
\begin{frame}
\frametitle{Show Protzko et al paper (published by now?)}
\end{frame}

\begin{frame}
\frametitle{Show data from Protzko $4 \times 4$ forest plots}
\end{frame}
...
\item For \alert{continuous outcomes}
 \begin{itemize}
\item    Difference in means $\theta$ between treatment groups
\end{itemize}
...
\begin{frame}
\frametitle{Example: Incidence of Preeclampsia in nine RCTs}
...
\begin{frame}[fragile]
\frametitle{Graphical Summary with 95\% CIs}
...
\frametitle{Estimate of Heterogeneity Variance and Higgins' $I^2$}
  \begin{itemize}
  \item \alert{Moment estimator} of heterogeneity variance compares
    $Q$-statistic to its expectation $n-1$ under homogeneity.
\item Truncation at zero and appropriate scaling gives 
  \[
