...
\begin{block}{}
  \begin{itemize}
    \item<1-> \color<6>{gray}{2015: Reproducibility project psychology}
    \item<2-> \color<6>{gray}{2016: Experimental economics replication project}
    \item<3-> \color<6>{gray}{2018: Experimental philosophy replicability project}
    \item<4-> \color<6>{red}{2018: Social sciences replication project}
    \item<5-> \color<6>{gray}{2021: Reproducibility Project: Cancer Biology}
  \end{itemize}
\end{block}
\begin{minipage}[t][5cm][t]{\textwidth}
  ...
  \only<5>{
  \includegraphics[width=0.6\textwidth]{images_presentation/errington2021.png}
  }
  \only<6>{
  \includegraphics[width=0.7\textwidth]{images_presentation/camerer2018.png}
  }
...
\begin{itemize}
  ...
  \Sexpr{round(study$fisr/study$fiso, 1)}$
...
@
    ...
    \begin{itemize}
      ...
      \item Minimum relative effect size
        \begin{itemize}
          \item With the two-trials rule: {\color{springgreen4} $\dmin = \Sexpr{round(dmin2TR, 2)} < d$}
          \item With the 
          sceptical $p$-value {\color{red} $\dmin = \Sexpr{round(dminRS, 2)} > d$}
        \end{itemize}
    \end{itemize}
      \pause
  \end{block}
