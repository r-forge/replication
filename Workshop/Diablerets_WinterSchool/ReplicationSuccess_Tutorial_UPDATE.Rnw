...
\begin{frame}{Assessing replication success}
  ...
  \pause
    ...
    \item[] One option: 
      ...
      \pause
      \item Use the {\color{red} meta-analytic result} as a single replication studies
      \pause
...
\begin{frame}[fragile]{Example for experiment FSD}
  \begin{block}{Accessing quantities}
  \end{block}
<<eval = T, results = 'hide'>>=


meta_FSD$TE.fixed # meta-analytic estimate

meta_FSD$seTE.fixed # standard error of estimate

meta_FSD$zval.fixed # meta-analytic z-value

# meta-analytic variance ratio (relative sample size)
FSD_ori <- subset(FSD, type == "original")
FSD_ori$se^2/meta_FSD$seTE.fixed^2
...
\begin{frame}[fragile]{Solution: Exercise 4.1}
  \framesubtitle{Forest plot FSD experiment}
<<echo = F>>=
forest(meta_FSD)
@
\end{frame}
