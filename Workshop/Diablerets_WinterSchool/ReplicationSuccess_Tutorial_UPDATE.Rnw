...
(thetar <- meta_FSD$TE.fixed) # meta-analytic estimate
(se_thetar <- meta_FSD$seTE.fixed) # standard error of estimate
(zr <- meta_FSD$zval.fixed) # meta-analytic z-value


FSD_ori <- subset(FSD, type == "original")
(c <- FSD_ori$se^2/se_thetar^2) # variance ratio (relative sample size)
