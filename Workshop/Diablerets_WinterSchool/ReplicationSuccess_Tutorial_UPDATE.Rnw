...
\begin{frame}{Shrinkage}
  ...
  \begin{block}{Shrinkage}
    \begin{itemize}
      \item is defined as {\color{foo} $s = 1 - d$}
...
<<echo = F>>=
dmin_sign <- effectSizeSignificance(zo = RProjects$zo,
                                    c = RProjects$c,
                                    level = 0.025)
dmin_rs <- effectSizeReplicationSuccess(zo = RProjects$zo,
                                        c = RProjects$c,
                                        level = 0.025,
                                        type = "golden",
                                        alternative = "one.sided")
dmin_DF <- as.data.frame(cbind(dmin_sign, dmin_rs,
                               RProjects$se_fiso^2/RProjects$se_fisr^2))
colnames(dmin_DF) <-  c("dmin_sign", "dmin_rs", "c")
                               
                               
ggplot(data= dmin_DF, aes(x = dmin_sign, y = dmin_rs)) +
   geom_point(aes(size  = c, sie = 10)) +
  xlim(0, 2) +
  ylim(0, 2) +
...
\begin{frame}{Power of the two-trials rule} 
  ...
  \begin{block}{Conditional design prior}
    \begin{eqnarray*}
  \mbox{Power} = \Phi\left({\color{foo} (1 - s)} z_o \sqrt{c} - z_{1- \alpha}\right)
    \end{eqnarray*}
  \end{block}
  \pause
    \begin{block}{Predictive design prior}
   ...
   \left( {\color{foo} (1 - s)} z_o \sqrt{c} - z_{1- \alpha}\right)\right)
    \end{eqnarray*}
  \end{block}
\end{frame}
