...
$\rightarrow$ Which threshold? \\
$\rightarrow$ {\color{tan3} one-sided $\alpha = 0.025^2 = 0.000625$} \\
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
<< fig.height = 4.5, echo = FALSE >>=
...
discrepant studies, as well as the minimum relative effect size $\dmin$ with the 
two approaches (two-trials rule and sceptical $p$-value). 
\\~\\
...
discTab2 <- xtable(discrepantDF[, -c(3, 4, 8:12)])
colnames(discTab2) <- c("Study", "$n_r / n_o$", "$p_o$",
                       "$p_r$", "$p_s$", "$d$", "$d_{min} (2TR)$", "$d_{min} (RS)$")
...
\section{More advanced topics}
  % \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\huge \textbf{Sequential replication studies}}
   \end{center}
  % \end{block}
\end{frame}
...
\begin{frame}{Interim power}
  ...
  \begin{block}{}
    \begin{itemize}
    \item Power of a replication study taking into account data from an interim analysis
    \item Conditional vs predictive 
    \item Depends on $p_o$, $c$, but also {\color{red} $p_i$ and $f$}
    \end{itemize}
  \end{block}
\end{frame}
