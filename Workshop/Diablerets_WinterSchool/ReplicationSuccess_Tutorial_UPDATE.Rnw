...
\includegraphics[width=0.6\textwidth]{images_presentation/CRS.png}
  ...
  \end{columns}
    \begin{center}
    {\color{darkred} Winter School 2022, Les Diablerets}
    \end{center}
\end{frame}
...
greycols <- c("orange", 6) ##adjustcolor(col = c("#1B1B1B", "#6D6D6D", "#BEBEBE"), alpha.f = 0.95)
...
##       rev(c("nominal", "golden", "2TR")),
       rev(c("sceptical p-value", "two-trials rule")),
       # col = mycol[sel], 
       col = rev(greycols),
       lty = 1, 
       lwd = 2, 
...
axis(4, 
     ...
     cex.axis = mycex, 
     col = greycols[2], # 6, ## mycol[2], 
     col.axis = greycols[2], # 6, ## mycol[2], 
     col.ticks = greycols[2], # 6 ## mycol[2]
     tck = -0.025) 
box()
...
abline(h = myorigPower*100, lty = 2, 
       col = "grey")
lines(myc, powerSig*100, col=greycols[2], lty=1, lwd=2)
...
\begin{frame}{Shrinkage penalization}
  \framesubtitle{in the sceptical $p$-value approach}
    \vspace{1cm}
  \begin{center}
...
\begin{frame}[fragile]{Solution: Exercise 1.5}
...
\begin{frame}[fragile]{Solution: Exercise 1.5 (extended)}
...
\begin{frame}{Power of the two-trials rule} 
  ...
  \pause
    \begin{block}{Predictive design prior}
    \begin{eqnarray*}
   \mbox{Power} = \Phi\left({\color{red} \sqrt{\frac{1}{c+1}}}
...
\begin{frame}{Power of the two-trials rule} 
  ...
  \pause
    \begin{block}{Predictive design prior}
    \begin{eqnarray*}
   \mbox{Power} = \Phi\left({\color{black} \sqrt{\frac{1}{c+1}}}
...
\begin{frame}{Power for replication success}
  \begin{block}{Conditional design prior}
    \begin{eqnarray*}
    \mbox{Power} = \Phi\left(\sqrt{c} z_o (1 - \dmin)\right)
    \end{eqnarray*}
  \end{block}
  ...
  \pause
    ...
    \begin{eqnarray*}
    \mbox{Power} = \Phi\left({\color{red} \sqrt{\frac{1}{c + 1}}}
    \left(\sqrt{c} z_o (1 - \dmin)\right)\right)
    \end{eqnarray*}
  \end{block}
\end{frame}
...
<<ch4b.0, echo = TRUE, fig.height=7.5, fig.width=9.75>>=
library(meta)
...
<<ch4b, echo = TRUE, fig.height=7.5, fig.width=9.75>>=
...
\end{block}
 ...
 Hint: Use the function \texttt{metagen} from the R package \texttt{meta}.
  
  \end{block}
  
  \begin{block}{Exercise 4.2}
  Compute Higgin's $I^2$ \\
  Hint:
  \end{block}
  
  \begin{block}{Exercise 4.3}
  Use the results from Exercise 4.1 to assess replication success with the
  two methods
  \end{block}
\end{frame}

\begin{frame}[fragile]{Solution: Exercise 4.1}
<<echo = T>>=

protzko_ext <- protzko2020[protzko2020$type == "external-replication",]
protzko_or <- protzko2020[protzko2020$type == "original",]

for(i in unique(protzko_ext$experiment)){
  dat = protzko_ext[protzko_ext$experiment == i, ]
...
\begin{frame}[fragile]{Solution: Exercise 4.2}

\makebox[1\textwidth][c]{  
  \resizebox{0.9\textwidth}{!} {
<<echo = F>>=

par(las = 1, mar = c(5.5, 5, 4, 2) + 0.1)
ma_mat <-   matrix(data = NA, nrow = length(unique(protzko_ext$experiment)), 
                  ncol = 4)

colnames(ma_mat) <-  c("I2", "zr", "sefixed", "experiment")

for(i in 1:length(unique(protzko_ext$experiment))){
  ex <- unique(protzko_ext$experiment)[i]
  dat = protzko_ext[protzko_ext$experiment == ex, ]
  ma <- metagen(TE = smd, seTE = se, data = dat, sm = "SMD")
  ma_mat[i, 1] <-  ma$I2*100
  ma_mat[i, 2] <- as.numeric(ma$zval.fixed)
...
\begin{frame}[fragile]{Solution: Exercise 4.3}

<<results = 'asis', echo = F>>=

zr_ma <- as.numeric(ma_mat[, 2])
pr_ma <- z2p(zr_ma, alternative = "one.sided") 
zo_ma <- protzko_or$smd/protzko_or$se
po_ma <- z2p(zo_ma, alternative = "one.sided")
c_ma = protzko_or$se^2/as.numeric(ma_mat[, 3])^2
pS_ma <- pSceptical(zo = zo_ma, zr = zr_ma, c = c_ma )


summary_ma <-  matrix(c(unique(protzko2020$experiment), 
                        formatPval(as.numeric(po_ma)), 
                        formatPval(as.numeric(pr_ma)), 
                        formatPval(as.numeric(pS_ma))), 
                      ncol = 4)
...
% \begin{itemize}
% \item Compute fixed- and random-effects meta-analysis of Protzko (external) replications
% \item Study heterogeneity estimates ($I^2$)
% \item Feed this into 2TR and sceptical $p$; how many replication successes with each method?
% \end{itemize}
...
\pause
\item[--] \textcolor{purple!50!red}{11 stopped} at stage 1
...
\begin{frame}{Interim power}
  ...
  \begin{block}{}
    ...
    {\bf data from an interim analysis}
    \item {\it Conditional} vs {\it predictive} 
    \item Depends on {\bf $p_o$}, {\bf $c$}, but also
      \begin{itemize}
...
\end{frame}
% ============================================================================
\nocite{Pyc2010, Opensc2015, Camerer2016, Camerer2018, Cova2018, Pawel2020, Held2020, Held2020c}
